...
async function askAma(q){
  ...
  // ═══ AMA backend resolution ═══
  // 1) remote proxy (Cloudflare Worker) for the live site
  // 2) window.claude.complete fallback (works inside the Claude design environment)
  // 3) offline keyword answers as the last-resort safety net
  const AMA_PROXY_URL = 'https://long-cell-cae6.sravanththatavarthi.workers.dev';
  ...
  // (1) Try the remote proxy first
  if(AMA_PROXY_URL){
    try {
      const ctrl = new AbortController();
      const timeoutId = setTimeout(()=>ctrl.abort(), 15000);
      const r = await fetch(AMA_PROXY_URL, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ question: q, context: SRAVANTH_CONTEXT }),
        signal: ctrl.signal,
      });
  ...
  // (2) Try in-environment Claude (worker has the strict system prompt; this path mirrors its rules inline)
  if(!reply && claudeApi){
    try {
      const prompt = `You are Sravanth.bot. ONLY answer questions about Sravanth Thatavarthi using the context below. Reply in 1-3 short sentences, no emojis, no filler. If off-topic, reply: "I only answer questions about Sravanth's work and background." If not in context, reply: "I don't have that detail — email Sravanth at sravanththatavarthi@gmail.com."\n\nCONTEXT:\n${SRAVANTH_CONTEXT}\n\n---\n\nQuestion: ${q}`;
